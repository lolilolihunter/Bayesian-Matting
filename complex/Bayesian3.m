...
NF= 0 ; NB = 0;
 ...
 %oriVar = temp / (height * width*255);  
   ...
   wBlock = width / 4 ;
   hBlock =  height/4;
   FBlockMean = zeros(4,4,3);
   BBlockMean = zeros(4,4,3);
   BlockcoF = zeros(4,4,3,3);
   BlockcoB = zeros(4,4,3,3);
   BlockVar = zeros(4,4);
   for i=0:3,
       for j=0:3,
                Ftemp = [0 0 0];
                ...
                oritemp = [0 0 0];
                oricount = 0;
                for a = floor(wBlock*i):floor(wBlock*(i+1))
                    for b = floor(hBlock*j):floor(hBlock*(j+1))
                         
                        try
                           if any(frontImg(a,b,:)),
                               Ftemp = Ftemp + frontImg(a,b,:);
                               Fcount = Fcount +1;
                           end
                           if any(backImg(a,b,:)),
                              Btemp = Btemp +backImg(a,b,:);
                              Bcount = Bcount +1;
                           end
                           oritemp = oritemp +oriImg(a,b,:);
                           oricount = oricount +1;
                        end                      
                    end
                end
                Ftemp = Ftemp / Fcount;
                Btemp = Btemp/Bcount;
                oritemp = oritemp / oricount;
                
                if Fcount ==0,
                     FBlockMean(i+1,j+1,:) =Fmean;    
                else
                    FBlockMean(i+1,j+1,:) =Ftemp; 
                end
                if Bcount==0,
                    BBlockMean(i+1,j+1,:) =Bmean;
                else
                   BBlockMean(i+1,j+1,:) =Btemp; 
                end
                
                coFTemp = [0 0 0;0 0 0;0 0 0];
                coBTemp = [0 0 0;0 0 0;0 0 0];
                NF= 0 ; NB = 0;
                temp = [0 0 0];
                for a = floor(wBlock*i):floor(wBlock*(i+1))
                    for b = floor(hBlock*j):floor(hBlock*(j+1))
                       ...
                       try
                        if any(frontImg(a,b,:)),
                            shiftF = [ (frontImg(a,b,1)-FBlockMean(i+1,j+1,1))  (frontImg(a,b,2)-FBlockMean(i+1,j+1,2))  (frontImg(a,b,3)-FBlockMean(i+1,j+1,3)) ];
                            coFTemp = coFTemp +(shiftF' * shiftF);
                            NF = NF +1;
                        end
                        if any(backImg(a,b,:)),
                            shiftB = [ (backImg(a,b,1)-BBlockMean(i+1,j+1,1))  (backImg(a,b,2)-BBlockMean(i+1,j+1,1))  (backImg(a,b,3)-BBlockMean(i+1,j+1,1)) ];
                            coBTemp = coBTemp + (shiftB' * shiftB);     
                            NB = NB +1;
                        end
                        temp = temp + ((reshape((oriImg(a,b,:)),1,3)-oritemp).^2);
                ...
                temp = sum(temp);
                temp = temp/ (wBlock*hBlock);
                BlockVar(i+1,j+1) = temp;
                if NF==0,
                coFTemp = coF;
                else
                coFTemp = coFTemp / NF;
                ...
                BlockcoF(i+1,j+1,:,:) = coFTemp;
                BlockcoB(i+1,j+1,:,:) = coBTemp;
       ...
       try 
         if any(unknownImg(a,b,:)),
               ...
               % oriVar = BlockVar(W,H);
                ...
                alpha = alpha / count;
                preAlpha = alpha;
                invcoF = inv(reshape(BlockcoF(W,H,:,:),3,3));
                invcoB = inv(reshape(BlockcoB(W,H,:,:),3,3));
                Fmean = reshape(FBlockMean(W,H,:),1,3);
                Bmean = reshape(BBlockMean(W,H,:),1,3);
                for i=1:Iteration,
